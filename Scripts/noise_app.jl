...
function compute_matrices(bins)
    ...
    𝐀 = (hcat(map(zˢ->Σ(k->T(zˢ,k,Λ,δ),Λ.m),δ.Z)...))'
    𝐀 = 𝐀[:,:]
    df2 = DataFrame(𝐀 = [𝐀], 𝐀⁻¹ = [inv(𝐀)], δ = [δ], Diff = true, Loss = false, bins = bins)

    T(zˢ,k,Λ,δ) = Ωext(δ.Z,zˢ/k).*δ.Tc(k,δ.Dp).*δ.Tl(Λ,δ.Dp)
    𝐀 = (hcat(map(zˢ->Σ(k->T(zˢ,k,Λ,δ),Λ.m),δ.Z)...))'
    𝐀 = 𝐀[:,:]
    df3 = DataFrame(𝐀 = [𝐀], 𝐀⁻¹ = [inv(𝐀)], δ = [δ], Diff = false, Loss = true, bins = bins)

    T(zˢ,k,Λ,δ) = Ωext(δ.Z,zˢ/k).*δ.Tc(k,δ.Dp)
    𝐀 = (hcat(map(zˢ->Σ(k->T(zˢ,k,Λ,δ),Λ.m),δ.Z)...))'
    𝐀 = 𝐀[:,:]
    df4 = DataFrame(𝐀 = [𝐀], 𝐀⁻¹ = [inv(𝐀)], δ = [δ], Diff = false, Loss = false, bins = bins)
...
function noise_app(Qcpc, N, Bins)
    i = (matrices[!,:bins] .== Bins) .& (matrices[!,:Diff] .== true) .& (matrices[!,:Loss] .== true)
    δ = matrices[i,:δ][1]
    𝐀 = δ.𝐀 
    𝐀i = inv(δ.𝐀) 
    ...
    df2 = DataFrame(Dp = 𝕟ⁱⁿᵛ.Dp, S = 𝕟ⁱⁿᵛ.S, Dist = ["𝕟ⁱⁿᵛ = 𝐀<sup>-1</sup> 𝕣" for i = 1:length(𝕣.Dp)])
    df = [df1; df2]
...
Qcpc = togglebuttons(OrderedDict("noise free"=>10.0, "0.05 Lpm"=>0.05, "0.3 Lpm"=>0.3,"1 Lpm"=>1.0), value = 0.3, label = "CPC Flow")
Ntot = togglebuttons(OrderedDict("300 cm-3"=>300.0, "3000 cm-3"=>3000.0,"30,000 cm-3"=>30000.0), value = 3000.0, label = "Total Number Concentration")
Bins = togglebuttons(OrderedDict("15"=>15, "30"=>30, "60"=>60, "120"=>120, "240"=>240,"480"=>480), value = 120, label = "Bins")
